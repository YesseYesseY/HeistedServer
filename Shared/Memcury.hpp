...
namespace Memcury
{
    ...
    public:
        ...
        auto ScanFor(const char* pattern, bool forward = true, int toSkip = 0) -> Scanner
        {
        ...
        auto ScanForEitherPattern(std::vector<const char*> patterns, bool forward = true, int toSkip = 0, int* foundIndex = nullptr) -> Scanner
        {
            const auto scanBytes = _address.GetAs<std::uint8_t*>();

            std::vector<std::vector<int>> opcodesToFindArr;
            for (auto pattern : patterns)
                opcodesToFindArr.push_back(ASM::pattern2bytes(pattern));
            ...
            for (auto i = (forward ? 1 : -1); forward ? (i < 2048) : (i > -2048); forward ? i++ : i--)
            {
                ...
                if (found)
                {
                    ...
                    if (toSkip != 0)
                    {
                        return ScanForEitherPattern(patterns, forward, toSkip - 1, foundIndex);
                    }
